        ...
        try {
          ...
          this.$matomo &&
            this.$matomo.trackEvent(
              "engagement",
              `jobseek_${
                /updateJobSeek/gi.test(jobSeekMutationType) ? "update" : "add"
              }`,
              `${this.jobSeek.title} | ${this.jobSeek.location} - ${jobSeekQueryResponse.data.data[jobSeekMutationType]._id}`
            );
