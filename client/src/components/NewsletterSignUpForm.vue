<template>
  ...
  >
    <p class="mb-3 h3" :style="titleBold ? 'font-weight: 700' : ''">
      {{ title }}
    </p>
    ...
    <div v-if="showJobSeekBtn" class="mt-3">
      <b-button
        block
        variant="primary"
    ...
    methods: {
      async onSubmit() {
        ...
        if (!this.formValidation()) {
          this.errors = [
            {
              message: "Bitte fülle die erforderlichen Felder aus!"
            }
          ];
          return null;
        }
      ...
      formValidation() {
        this.validated = true;
        return !this.form.email || !this.form.state || !this.form.accepted
          ? false
          : true;
      }
    }
